        ...
        >
          ...
          <div className="sticky top-0 flex items-center justify-between p-6 border-b border-slate-200 bg-white">
            <div className="flex-1">
              <h2 className="text-xl font-bold text-slate-900">
                {record.membershipSociety || "Technical Body Membership"}
              </h2>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase mb-2">Membership Status</p>
                <StatusBadge status={record.membership} />
              </div>
            </div>
            ...
            {record.membership === "yes" && (
              <div className="border-t border-slate-200 pt-6">
                <h3 className="font-semibold text-slate-900 mb-4">Membership Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 rounded-lg p-4">
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Level of Membership</p>
                    <p className="text-slate-900 font-medium capitalize">{record.levelOfMembership}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">State of Membership</p>
                    <p className="text-slate-900 font-medium capitalize">{record.stateOfMembership}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Membership Number</p>
                    <p className="text-slate-900 font-medium">{record.membershipNumber}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Membership Society</p>
                    <p className="text-slate-900 font-medium">{record.membershipSociety}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Validity</p>
                    <p className="text-slate-900 font-medium">{record.validity}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Charges (₹)</p>
                    <p className="text-slate-900 font-medium">{record.chargesInRupees}</p>
                  </div>
                </div>

                {/* Activities Section */}
                {record.activitiesConducted === "yes" && (
                  <div className="mt-6 pt-6 border-t border-slate-200">
                    <h3 className="font-semibold text-slate-900 mb-4">Activity Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 rounded-lg p-4">
                      <div>
                        <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Activity</p>
                        <p className="text-slate-900 font-medium">{record.specifyActivity}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-500 uppercase mb-1">Activity Status</p>
                        <p className="text-slate-900 font-medium capitalize">{record.activityStatus}</p>
                      </div>
                    </div>
                  </div>
                )}
...
export default function TechnicalBodyMembershipPage() {
  ...
  useEffect(() => {
    const loadRecords = async () => {
      try {
        setLoading(true);
        const response = await apiClient.get("/student-technical-body-memberships?page=1&limit=50");
        setRecords(response?.records || []);
      } catch (err: any) {
        console.error("Failed to load technical body memberships:", err);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter(
      (item) =>
        ...
        (item.membershipSociety?.toLowerCase().includes(normalized)) ||
        item.membership.toLowerCase().includes(normalized)
    );
  }, [query, records]);
  ...
  const handleUpdateIqacStatus = async (id: number, currentStatus: string) => {
    ...
    try {
      setUpdatingId(id);
      await apiClient.put(`/student-technical-body-memberships/${id}/iqac-status`, {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Technical Body Membership</h1>
              <p className="text-sm text-slate-500 mt-1">View and manage your membership records</p>
            </div>
          </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-center gap-3 bg-white rounded-lg border border-slate-200 shadow-sm p-4">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by student name, ID, or membership society..."
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-slate-600 font-medium">Loading technical body memberships...</p>
            </div>
          </div>
          ...
          <div className="text-center py-12 bg-white rounded-lg border-2 border-dashed border-slate-300">
            ...
            <p className="text-slate-600 mb-6">Start by adding your first membership record.</p>
            <Link
              href="/student/technical-body-membership/submit"
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="mb-4 pb-3 border-b border-slate-100">
                    ...
                    <div className="space-y-2">
                      <div>
                        <p className="text-xs text-slate-600 mb-1">Membership</p>
                        <StatusBadge status={record.membership} />
                      </div>
                      <div>
                  ...
                  {record.membership === "yes" && (
                    <div className="mb-4 pb-3 border-b border-slate-100">
                      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Membership Details</p>
                      <div className="space-y-1 text-xs">
                        <p className="text-slate-900"><span className="font-medium">Level:</span> {record.levelOfMembership}</p>
                        <p className="text-slate-900"><span className="font-medium">State:</span> {record.stateOfMembership}</p>
                        {record.membershipNumber && <p className="text-slate-900"><span className="font-medium">Number:</span> {record.membershipNumber}</p>}
                      </div>
                    </div>
                  )}
