...
type JournalRecord = {
  ...
  sponsorship_other_specify?: string;
...
type FormState = {
  ...
  // ── Indexed ──
  paperIndexed: string;
  indexedDetails: string;        // Scopus | SCI/SCI(E) | WoS | Others
  indexedOtherDetails: string;   // free-text when "Others"
  // ── Impact factor ──
  impactFactor: string;
  impactFactorValue: string;     // numeric value when "Yes"
  studentAuthorPosition: string;
  labsInvolved: string;
  projectOutcome: string;
  // ── Sponsorship ──
  sponsorshipType: string;       // Self | Institute | Others
  sponsorshipAmount: string;     // shown for Institute & Others
  sponsorshipOtherSpecify: string; // shown only for Others
  // ── Interdisciplinary ──
...
export default function JournalPublicationPage() {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((prev) => {
      ...
      // Reset indexed sub-fields when paperIndexed changes
      ...
      // Reset impact factor value when impactFactor changes
      if (name === "impactFactor" && value !== "Yes") {
        next.impactFactorValue = "";
      }

      // Reset sponsorship sub-fields when sponsorshipType changes
  ...
  const handleOpenDetail = async (record: JournalRecord) => {
    setSelected(record);
    setDetailLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/journal-publications/${record.id}`);
      if (!response.ok) throw new Error("Failed to load record details");
      const data = (await response.json()) as JournalRecord;
      setSelected(data);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [recordsData, studentsData, labsData, departmentsData] = await Promise.all([
          fetchJson<JournalRecord[]>("http://localhost:5000/api/journal-publications"),
        ...
        setStudents(studentsData.map((s) => ({ ...s, student_name: getStudentLabel(s) })));
        setLabs(labsData);
        ...
        <div className="mx-auto max-w-7xl space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button type="button" className="rounded-lg border border-gray-200 bg-white p-2 text-gray-400 hover:bg-gray-50 hover:text-gray-600 transition-colors">
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3">
              <div className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 min-w-[220px]">
                ...
                <input type="text" placeholder="Search publications, students…" className="w-full bg-transparent text-xs text-gray-700 placeholder:text-gray-400 outline-none" />
              </div>
              <button type="button" className="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-medium text-gray-500 hover:bg-gray-50 transition-colors">
            ...
            {recordsLoading ? (
              ...
              <>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/80">
                        {["Student / Paper", "Journal", "Volume", "Issue", "DOI", "Indexed", "Impact Factor", "Published Date", "IQAC", ""].map((h) => (
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {records.map((record) => {
                        ...
                        return (
                          <tr key={record.id} onClick={() => handleOpenDetail(record)} className="group hover:bg-indigo-50/40 cursor-pointer transition-colors">
                            ...
                            <td className="px-4 py-3.5 max-w-[150px]"><p className="text-gray-600 truncate text-[11px]">{record.journal_name}</p></td>
                ...
                {selected && (
                  <>
                    <div onClick={() => setSelected(null)} className="fixed inset-0 z-40 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-150" />
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
                          ...
                          <button onClick={() => setSelected(null)} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors shrink-0">
                            <X size={16} />
                          </button>
                        </div>
                            ...
                            <>
                              <ModalSection title="Student & Paper Info">
                                <div className="grid grid-cols-2 gap-3">
                                  ...
                                  <InfoCell label="SDG Goal" value={selected.sdg_title ?? SDG_MAPPING[selected.sdg_goals] ?? `SDG ${selected.sdg_goals}`} />
                                </div>
                              </ModalSection>
                              ...
                              <ModalSection title="Links & Files">
                                <div className="rounded-xl bg-gray-50 border border-gray-100 px-4 py-3">
                                  ...
                                  <a href={selected.web_url} target="_blank" rel="noreferrer" className="text-xs font-medium text-indigo-600 hover:underline break-all">
                                    {selected.web_url}
                                  </a>
                                </div>
                              </ModalSection>
                              ...
                              <ModalSection title="Paper Properties">
                                <div className="grid grid-cols-2 gap-3">
                                  ...
                                  {selected.paper_indexed === "Yes" && selected.indexed_details && (
                                  ...
                                  {(selected.sponsorship_type === "Institute" || selected.sponsorship_type === "Others") && selected.sponsorship_amount && (
                                    <InfoCell label="Sponsorship Amount" value={`₹ ${selected.sponsorship_amount}`} />
                                  )}
                                  {selected.sponsorship_type === "Others" && selected.sponsorship_other_specify && (
                                    <InfoCell label="Sponsorship — Other Details" value={selected.sponsorship_other_specify} />
                                  )}
                                  <InfoCell label="Interdisciplinary" value={selected.interdisciplinary ? "Yes" : "No"} />
                                  {selected.interdisciplinary === 1 && selected.interdisciplinary_dept_name && (
                                    <InfoCell label="Other Department" value={selected.interdisciplinary_dept_name} />
                                  )}
                                  {selected.interdisciplinary === 1 && selected.other_dept_student_count != null && (
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  {[
                                    ...
                                    <button
                                      ...
                                      className={`group block rounded-xl overflow-hidden border transition-all text-left w-full ${url ? "border-gray-200 hover:border-indigo-300 hover:shadow-md cursor-pointer" : "border-gray-100 opacity-50 cursor-not-allowed"}`}
                                      disabled={!url}
                                    >
                                      <div className="relative bg-gray-100 h-40 overflow-hidden flex items-center justify-center">
                                        {url && /\.(png|jpe?g|webp|gif|bmp|svg)(\?.*)?$/i.test(url) ? (
                                          <>
                                            <img src={url} alt={label} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                        ...
                        {lightbox && (
                          <div onClick={() => setLightbox(null)} className="fixed inset-0 z-[60] bg-black/85 flex items-center justify-center p-6">
                            <div className="relative max-w-[90vw] max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
                              <button onClick={() => setLightbox(null)} className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/60 hover:bg-black/85 text-white flex items-center justify-center transition-colors">
                                <X size={16} />
                              </button>
                              <img src={lightbox.url} alt={lightbox.label} className="block max-w-[90vw] max-h-[85vh] object-contain rounded-xl" />
                        ...
                        <div className="px-6 py-4 border-t border-gray-100 shrink-0 flex justify-end">
                          <button onClick={() => setSelected(null)} className="px-5 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm font-medium text-gray-600 hover:bg-gray-100 transition-colors">
                            Close
                          </button>
                        </div>
                      </div>
      ...
      <div className="mx-auto max-w-7xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="border-b border-gray-100 px-6 py-5 sm:px-8">
          <nav className="flex flex-wrap items-center gap-1.5 text-xs text-gray-400 mb-4">
            ...
            <button type="button" onClick={handleCancel} className="font-medium text-indigo-500 hover:text-indigo-700 transition-colors">
          ...
          <div className="flex items-start justify-between gap-4 mb-5">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="rounded-full border border-indigo-200 bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-600">Draft</span>
              <button type="button" onClick={handleCancel} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                <X size={13} />
                Close
              </button>
            </div>
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                <div key={step.id} className="flex items-center flex-1 last:flex-none">
                  <div className="flex items-center gap-2">
                    <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-medium transition-colors ${isDone ? "bg-emerald-500 text-white" : isActive ? "bg-indigo-600 text-white" : "border border-gray-200 bg-white text-gray-400"}`}>
                      {isDone ? <Check size={11} strokeWidth={2.5} /> : step.id}
                    </div>
                    <span className={`hidden sm:block text-xs transition-colors ${isActive ? "font-medium text-gray-800" : "text-gray-400"}`}>{step.label}</span>
                  </div>
                  {i < STEPS.length - 1 && <div className={`mx-3 h-px flex-1 transition-colors ${isDone ? "bg-emerald-400" : "bg-gray-200"}`} />}
                </div>
              );
            })}
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 sm:px-8 sm:py-7 space-y-7">
          ...
          <FormSection icon={<GraduationCap size={14} />} title="Student information" onActivate={() => setActiveStep(1)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <SearchableSelect label="Student" required name="student" value={form.student} placeholder="Choose student"
                options={students.map((s) => ({ value: String(s.id), label: getStudentLabel(s) }))}
                onChange={handleSearchableChange}
              />
              <SelectField label="Year of Study" required name="yearOfStudy" value={form.yearOfStudy}
                options={["Choose year", "First", "Second", "Third", "Fourth"]} onChange={handleChange}
              />
              <SearchableSelect label="Special Lab" required name="specialLab" value={form.specialLab} placeholder="Choose lab"
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-4 space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Author breakdown</span>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  ...
                  {showStudentCountField && (
                    <div>
                      ...
                      <select value={form.studentAuthorCount} onChange={handleStudentAuthorCountChange} className="w-full appearance-none rounded-lg border border-gray-200 bg-white bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%239CA3AF%22%20stroke-width%3D%222%22%3E%3Cpath%20d%3D%22M6%209l6%206%206-6%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[right_10px_center] px-3 py-2 pr-8 text-sm text-gray-800 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
                        <option value="">Choose</option>
                        {Array.from({ length: totalAuthorsNum + 1 }, (_, i) => <option key={i} value={String(i)}>{i}</option>)}
                      </select>
                  ...
                  {showFacultyCountField && (
                    <div>
                      <label className="mb-1.5 block text-xs font-medium text-gray-700">Number of faculty authors</label>
                      <input readOnly value={form.facultyAuthorCount} className="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm text-gray-500 cursor-default outline-none" />
                  ...
                  <div className="space-y-3">
                    {studentCountNum > 0 && (
                      <div className="rounded-lg border border-indigo-100 bg-indigo-50/40 p-3 space-y-3">
                        <p className="text-[10px] font-semibold text-indigo-500 uppercase tracking-widest">Student author names</p>
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                          {form.studentAuthorNames.map((name, i) => (
                            <div key={i}>
                              <label className="mb-1.5 block text-xs font-medium text-gray-700">Student author {i + 1}<span className="ml-0.5 text-red-500">*</span></label>
                              <input type="text" value={name} placeholder="Full name as in paper" onChange={(e) => handleStudentAuthorNameChange(i, e.target.value)} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 outline-none transition placeholder:text-gray-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {facultyCountNum > 0 && (
                      <div className="rounded-lg border border-emerald-100 bg-emerald-50/40 p-3 space-y-3">
                        <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-widest">Faculty author names</p>
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                          {form.facultyAuthorNames.map((name, i) => (
                            <div key={i}>
                              <label className="mb-1.5 block text-xs font-medium text-gray-700">Faculty author {i + 1}<span className="ml-0.5 text-red-500">*</span></label>
                              <input type="text" value={name} placeholder="Full name as in paper" onChange={(e) => handleFacultyAuthorNameChange(i, e.target.value)} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 outline-none transition placeholder:text-gray-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <FormSection icon={<Link size={14} />} title="Publication information" onActivate={() => setActiveStep(3)}>
            <div className="space-y-4">
              ...
              {/* ── Paper Indexed + conditional sub-fields ── */}
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                {showIndexedDetails && (
                  <SelectField label="Indexed Details" required name="indexedDetails" value={form.indexedDetails}
                    options={["Choose", ...INDEXED_OPTIONS]} onChange={handleChange}
                  />
                )}
                ...
                <SelectField label="Project Outcome" required name="projectOutcome" value={form.projectOutcome} options={["Choose", "Yes", "No"]} onChange={handleChange} />
              </div>

              {/* ── SDG Goals ── */}
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                <SearchableSelect label="SDG Goals" required name="sdgGoals" value={form.sdgGoals} placeholder="Choose SDG"
                  options={Object.entries(SDG_MAPPING).map(([value, label]) => ({ value, label }))}
                  onChange={handleSearchableChange}
                />
              </div>

              {/* ── Sponsorship Type + conditional sub-fields ── */}
              <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-4 space-y-4">
                <div className="flex items-center gap-2">
                  <div className="flex items-center justify-center w-5 h-5 rounded-md bg-gray-100 text-gray-400">
                    <Users size={11} />
                  </div>
                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Sponsorship details</span>
                  <div className="flex-1 h-px bg-gray-200" />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <SelectField
                    ...
                    options={["Choose", "Self", "Institute", "Others"]}
                    onChange={handleChange}
                  />

                  {showSponsorshipAmount && (
                    <InputField
                      label={form.sponsorshipType === "Institute" ? "Sponsorship Amount (₹)" : "Amount (₹)"}
                      required
                      type="number"
                  ...
                  {showSponsorshipOther && (
                    <InputField
                      ...
                      required
                      name="sponsorshipOtherSpecify"
                      value={form.sponsorshipOtherSpecify}
                      placeholder="Specify sponsorship source"
                      onChange={handleChange}
                    />
                  )}
                </div>
              ...
              {/* ── Interdisciplinary + conditional sub-fields ── */}
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
              ...
              {showInterdisciplinary && (
                <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-semibold text-violet-500 uppercase tracking-widest">Interdisciplinary details</span>
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 border-t border-gray-100 pt-5">
            <button type="button" onClick={handleCancel} disabled={loading} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button type="button" disabled={loading} className="rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-100 transition-colors disabled:opacity-50">
              Create &amp; Add Another
            </button>
            <button type="submit" disabled={loading} className="flex items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition-colors disabled:opacity-50">
              {loading ? (
                <><span className="h-3.5 w-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin" />Creating…</>
              ) : (
                <><Check size={14} />Create Record</>
              )}
            </button>
          </div>
...
function FormSection({ icon, title, children, onActivate }: { icon: React.ReactNode; title: string; children: React.ReactNode; onActivate?: () => void }) {
  return (
    <div className="space-y-4" onClick={onActivate}>
      <div className="flex items-center gap-2">
        <div className="flex items-center justify-center w-6 h-6 rounded-md border border-gray-200 bg-gray-50 text-gray-500">{icon}</div>
...
function FileField({ label, required, name, onChange, fileName }: FileFieldProps) {
  return (
    <div>
      ...
      <label className={`group flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed px-4 py-5 text-center transition-colors ${fileName ? "border-emerald-300 bg-emerald-50" : "border-gray-300 bg-gray-50 hover:border-indigo-300 hover:bg-indigo-50"}`}>
        <div className={`flex items-center justify-center rounded-lg p-2 transition-colors ${fileName ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-400 group-hover:bg-indigo-100 group-hover:text-indigo-500"}`}>
