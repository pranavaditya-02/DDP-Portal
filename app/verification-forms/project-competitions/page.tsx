...
type SortKey = 'student_name' | 'competition_name' | 'competition_type' | 'event_level' | 'iqac_verification' | 'created_at';
type SortDirection = 'asc' | 'desc' | null;

export default function ProjectCompetitionsVerificationPage() {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        setLoading(true);
        const response = await apiClient.get('/student-project-competitions?page=1&limit=1000');
        setRecords(response.records || []);
      } catch (error) {
        console.error('Failed to fetch records:', error);
        toast.error('Failed to load project competition records');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      return (
        ...
        record.competition_name?.toLowerCase().includes(normalized) ||
        record.competition_type?.toLowerCase().includes(normalized) ||
  ...
  const handleUpdateStatus = async (id: number, status: 'Approved' | 'Rejected', reason?: string) => {
    try {
      ...
      await apiClient.put(`/student-project-competitions/${id}/iqac-status`, payload);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Project Competitions - IQAC Verification</h1>
          <p className="text-slate-600">Review and approve/reject project competition submissions</p>
        </div>
        ...
        <div className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by ID, student, competition, type, level or status..."
              value={query}
            ...
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-slate-500">
                  ...
                  >
                    Competition
                    {computeSortIcon('competition_name')}
                  </button>
                </th>
                <th className="px-4 py-3 text-left font-medium text-slate-500">
                  <button
                  ...
                  >
                    Type
                    {computeSortIcon('competition_type')}
                  </button>
                </th>
                <th className="px-4 py-3 text-left font-medium text-slate-500">
                  <button
              ...
              {loading ? (
                ...
                sortedRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">{record.competition_name || 'N/A'}</td>
                    <td className="px-4 py-3 text-slate-700">{record.competition_type || 'N/A'}</td>
        ...
        >
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">Project Competition Details</h2>
            ...
            <div className="space-y-4 px-6 py-6 overflow-y-auto max-h-[70vh]">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <div className="text-xs uppercase tracking-wide text-slate-500">Competition Name</div>
                  <div className="mt-1 font-medium text-slate-900">{selectedRecord.competition_name || 'N/A'}</div>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <div className="text-xs uppercase tracking-wide text-slate-500">Competition Type</div>
                  <div className="mt-1 font-medium text-slate-900">{selectedRecord.competition_type || 'N/A'}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="mt-1 font-medium text-slate-900">{selectedRecord.event_mode || 'N/A'}</div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wide text-slate-500">Result Type</div>
                  <div className="mt-1 font-medium text-slate-900">{selectedRecord.result_type || 'N/A'}</div>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <div className="text-xs uppercase tracking-wide text-slate-500">Competition Date</div>
                  <div className="mt-1 font-medium text-slate-900">
                    {selectedRecord.competition_date ? new Date(selectedRecord.competition_date).toLocaleDateString() : 'N/A'}
                  </div>
                </div>
                <div>
                  ...
                  {isConfirmingReject ? (
                    <div className="space-y-3 rounded-2xl border border-rose-200 bg-rose-50 p-4">
                      ...
                      <textarea
                        ...
                        placeholder="Please explain why this project competition is rejected."
                      />
