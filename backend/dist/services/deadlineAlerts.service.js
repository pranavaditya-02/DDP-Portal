...
class DeadlineAlertsService {
     ...
     */
    getDaysUntilDeadline(deadlineISO) {
     ...
     */
    hasAlertBeenSent(facultyId, taskId) {
     ...
     * Mark alert as sent
     */
    markAlertAsSent(facultyId, taskId) {
        const key = `${ALERT_SENT_KEY_PREFIX}${facultyId}_${taskId}`;
     ...
     */
    async sendDeadlineAlertsForFaculty(facultyId, facultyEmail, facultyName, deadlines) {
        let sent = 0;
        let skipped = 0;
        for (const deadline of deadlines) {
            ...
            if (daysUntilDeadline <= 7 && daysUntilDeadline > 0) {
                if (this.hasAlertBeenSent(facultyId, deadline.taskId)) {
                    logger.info(`Alert already sent today for ${deadline.taskId} to faculty ${facultyId}`);
                    skipped++;
                    continue;
                }
                // Format deadline for display
                const deadlineDate = new Date(deadline.deadlineISO).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
                // Send email alert
                const emailSent = await emailService.sendDeadlineAlert(facultyEmail, facultyName, deadline.taskTitle, deadlineDate, daysUntilDeadline);
                if (emailSent) {
