...
router.post('/check-and-send', async (req, res) => {
    try {
        ...
        const result = await deadlineAlertsService.sendDeadlineAlertsForFaculty(facultyId || req.user?.id || 1, facultyEmail || req.user?.email || 'unknown@local.dev', facultyName || req.user?.email?.split('@')[0] || 'Faculty Member', upcomingDeadlines);
        res.json({
...
router.post('/send-bulk', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await deadlineAlertsService.sendDeadlineAlertsForFaculty(facultyId, facultyEmail, facultyName, deadlines);
        res.json({
...
router.post('/task-completed', async (req, res) => {
    try {
        ...
        const ok = await emailService.sendTaskCompletionNotification(recipient, senderName, senderEmail, taskTitle, doneAt);
        if (!ok) {
