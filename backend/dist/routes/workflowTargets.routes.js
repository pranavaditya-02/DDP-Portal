...
router.get('/admin/deadlines', authenticateToken, requireRole('maintenance', 'admin'), async (req, res) => {
    try {
        const parsed = academicYearSchema.safeParse(req.query.academicYear || '2026-27');
        if (!parsed.success) {
            return res.status(400).json({ error: parsed.error.errors });
...
router.put('/admin/deadlines', authenticateToken, requireRole('maintenance', 'admin'), async (req, res) => {
    try {
...
router.get('/me/workflow', authenticateToken, async (req, res) => {
    try {
        const parsed = academicYearSchema.safeParse(req.query.academicYear || '2026-27');
        if (!parsed.success) {
            return res.status(400).json({ error: parsed.error.errors });
...
router.post('/me/workflow/complete', authenticateToken, async (req, res) => {
    try {
